...
describe('Count routes', () => {
  it("#6: Count routes C --> C with max 3 stops is 2", () => {
    assert.equal(2, railroad.countRoutesLessThan('C', 'C', 3))
  })

  it("#7: Count routes A --> C with exactly 4 stops is 3", () => {
    assert.equal(3, railroad.countRoutesEqualsTo('A', 'C', 4))
  })
  
  it("#10: Count routes C --> C with max distance 30 is 7", () => {
    assert.equal(7, railroad.countRoutesWithDistanceMax('C', 'C', 30))
  })
})
  
describe('Shortest distance', () => {
  it("#8: Shortest distance of A --> C is 9", () => {
    assert.equal(9, railroad.getDistanceShortestRoute('A', 'C'))
  })

  it("#9: Shortest distance of B --> B is 9", () => {
    assert.equal(9, railroad.getDistanceShortestRoute('B', 'B'))
  })
